...
class ProportionalResourceAllocator(AbstractResourceAllocator):
    def __init__(self):
        super().__init__()
    ...
    def __recompute_buckets(
        ...
        if boundaries is None and importance_factors is None:
            boundaries = [1 / x for x in range(bucket_count, 0, -1)]
            boundaries.insert(0, 0)
            boundaries.append(3)
            importance_factors = [x for x in range(1, bucket_count - 1)]
                    ...
                    [
                        {
                            "$bucket": {
                                "groupBy": {
                                    "$avg": f"${str.replace(metric_path, '__', '.')}"
                                },
